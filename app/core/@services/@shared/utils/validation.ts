...
const validateValue = (
  ...
  onFailOptions?: OnValueValidateFailOptions,
) => {
...
const onValueValidateFail = (query: string, options: OnValueValidateFailOptions = {}) => {
  const { message = "This field is required", insertAfterQuery = `${query}` } = options;
  ...
  if (containerElement) {
    ...
    if (insertAfterELement) {
      ...
      p.setAttribute("data-message", message);
  ...
  } else console.error("element not found to apply warning-styles", query);
};
